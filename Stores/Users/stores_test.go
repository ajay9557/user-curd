...
func TestGetEmail(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: true,
			mockCall: mock.ExpectQuery("Select * from HUser where email=?").WithArgs("ridhdhish@gmail.com").WillReturnRows(rows),
		},
		{
			desc:     "Case2",
			email:    "naruto@gmail.com",
			expected: false,
			mockCall: mock.ExpectQuery("Select * from HUser where email=?").WithArgs("naruto@gmail.com").WillReturnError(errors.New("No Mail ID exists")),
		},
	}
